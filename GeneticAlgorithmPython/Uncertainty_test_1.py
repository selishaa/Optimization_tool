...
BOUND_LOW, BOUND_UP = [25,1,1,1,5,1,5,5],[300,15,10,10,15,20,15,15]      # The boundaries for all variables
...
# Means
pv_mean=np.mean(P_pv)
wind_mean=np.mean(p_wind)
hydro_mean=np.mean(hydro_gen)
bio_mean=np.mean(biogas_gen)
    
# Variance
pv_var=np.var(P_pv)
wind_var=np.var(p_wind)
hydro_var=np.var(hydro_gen)
bio_var=np.var(biogas_gen)

    
load_means=np.mean(load)
load_variances=np.var(load)

    # Define the parameters for renewable energy sources (mean and variance)
renewable_means = {
    'solar': pv_mean,  # in kWh
    'wind': wind_mean,   # in kWh
    'hydropower': hydro_mean,  # in kWh
    'biogas': bio_mean   # in kWh
}

renewable_variances = {
    'solar': pv_var,
    'wind': wind_var,
    'hydropower': hydro_var,
    'biogas': bio_var
}
# Define the number of simulations
num_simulations = 100

    # Generate random samples for multi-energy loads and renewable energy sources
loads_samples = {energy_type: np.random.normal(load_means[energy_type], np.sqrt(load_variances[energy_type]), num_simulations) for energy_type in load_means}
renewable_samples = {energy_type: np.random.normal(renewable_means[energy_type], np.sqrt(renewable_variances[energy_type]), num_simulations) for energy_type in renewable_means}

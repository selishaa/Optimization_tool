#version of SIRES with elitism (doesn't allow Django to access functions with elitism but it can access functions 
# with info about different energy sources)
...
p_wind = np.zeros(24)
wind = np.zeros(24)
def set_wind(velocity):
    global v
    v = velocity

# Hydro power generation
def set_hydro(base_h):
    global hydro_base
    hydro_base = base_h
    
g = 9.8
hydro_gen = np.zeros(24)
hydro_ele = np.zeros(24)
Hd = 50
n_hydro = 0.7
...
W_irrigation = 80 + 100 * random.uniform(0,1) * np.ones(24)
...
def get_hydro():
    x_3 = 5
    for t in range(24):
        hydro_ele[t] = hydro_base +(5 * random.uniform(0,1))
        hydro_gen[t] = x_3* n_hydro * 1 * Hd * g *hydro_ele[t]/3600
        return hydro_gen
def get_bio():
    x_4 = 4
    for t in range(24):
        bio_elec[t] = bio_base + random.uniform(0,1)
...
def feasible(individual):
    ...
    results = [LPSP, LWSP, ACS, InitialCost, NPC]
    return results 
...
def main():
    ...
    # plot biogas
    axes[0,3].plot(np.arange(24), biogas_gen)
    ...
    axes[1,0].plot(np.arange(24), Q_bio)
    ...
    axes[1,1].plot(np.arange(24), W_wind)
    ...
    axes[1,2].plot(np.arange(24), Q_pv)
...
def generate_pie_chart1(P_pv_ele, P_wind_ele, hydro_gen_ele, biogas_gen_ele):
    resource_sums = {
...
def generate_pie_chart2(Q_bio_pump, Q_pv_pump, W_wind_pump):
    resource_sums = {
